...
abstract class Base
{
    ...
    {
        if (!defined('XIAOCMS_MEMBER') || defined('XIAOCMS_ADMIN')) {
            return false;
        }
    ...
    protected function post_check_fields($fields, $data)
    {
        foreach ($fields as $t) {
            ...
            if ($t['pattern']) {
                if ($t['pattern'] == 1) {
                    if ($data[$t['field']] == '') {
                        $this->show_message(empty($t['errortips']) ? $t['name'] . '不能为空' : $t['errortips'], 2, 1);
                    }
                } else {
                    if (!preg_match($t['pattern'], $data[$t['field']])) {
                        $this->show_message(empty($t['errortips']) ? $t['name'] . '格式不正确' : $t['errortips'], 2, 1);
                    }
                }
            }
            if ($t['formtype'] == 'related') {
                ...
                foreach ($data[$t['field']] as $k => $v) {
                    if (!$v) {
                        unset($data[$t['field']][$k]);
                    }
                }
                $data[$t['field']] = implode(',', $data[$t['field']]);
            }
            if (is_array($data[$t['field']])) {
    ...
    {
        foreach ($fields as $t) {
            if (in_array($t['formtype'], array('checkbox', 'files', 'diy'))) {
