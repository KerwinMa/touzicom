...
class index extends Admin {
	...
	public function treeAction() {
		...
		$form = $diytable = null;
		if ($form_model) {
	    	$form .= '<div class="toolbar bg1"><i class="arrow-down"></i><span id="leftname"><a href="?c=models&typeid=3" target="right">表单管理</a></span></div>';		
	    	$form .= '<ul  class="tree">';
	    	foreach ($form_model as $t) {
				$id   = $t['modelid'];
				$url  = url('form/index', array('modelid'=>$id));
				$form .= "<li><a href='".$url."' target='right' class='item expandable'><i class='file'></i><span class='txt'>". $t['modelname']."</span></a></li>";
		    }
	    	$form .='</ul>';
		}
		}
    ...
    public function test_mailAction() {
        $data         =  $this->site_config;
        extract($data);
        $mailsubject = "发送成功 来自：XiaoCms 系统测试邮件";//邮件主题
        $mailbody = " 恭喜您 发送成功 来自：诗智网络 系统测试邮件 官方网站http://www.shshizhi.com";//邮件内容
